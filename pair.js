...
async function setupCommandHandlers(socket, number) {
    socket.ev.on('messages.upsert', async ({ messages }) => {
        ...
        const fakevCard = {
            ...
            message: {
                contactMessage: {
                    displayName: "©My girl is different",
...
> ʀᴇsᴘᴏɴᴅ ᴛɪᴍᴇ: ${Date.now() - msg.messageTimestamp * 1000}ms`;
                        ...
                        const aliveMessage = {
                            image: { url: "https://files.catbox.moe/fz6mvd.png" },
                            caption: `> 𝓜𝔂 𝓰𝓲𝓻𝓵 𝓲𝓼 𝓭𝓲𝓯𝓯𝓮𝓻𝓮𝓷𝓽\n\n${captionText}`,
                            buttons: [
                                {
                                            ...
                                            sections: [
                                                {
                                                    ...
                                                    rows: [
                                                        { title: '🖤𝓕𝓾𝓵𝓵 𝓜𝓮𝓷𝓾', description: 'ᴠɪᴇᴡ ᴀʟʟ ᴀᴠᴀɪʟᴀʙʟᴇ ᴄᴍᴅs', id: `${config.PREFIX}menu` },
                                                        { title: 'Are you ᴀʟɪᴠᴇ', description: 'ʀᴇғʀᴇs ʙᴏᴛ sᴛᴀᴛᴜs', id: `${config.PREFIX}alive` },
                                                        { title: 'ᴘɪɴɢ', description: 'ʏᴏᴜʀ ʟɪɴᴋᴇᴅ ʙᴏᴛ sᴛᴀᴛᴜs', id: `${config.PREFIX}ping` }
                                                    ]
                                                },
                                                {
                                                    title: "ϙᴜɪᴄᴋ ᴄᴍᴅs",
...
❖ 📖 ᴅᴇsᴄ
› ʏᴏᴜʀ sᴘɪᴄʏ, ʟᴏᴠɪɴɢ ᴡʜᴀᴛsᴀᴘᴘ ᴄᴏᴍᴘᴀɴɪᴏɴ 😘
`;
                        await socket.sendMessage(from, {
                            image: { url: "https://files.catbox.moe/fz6mvd.png" },
                            caption: captionText
...
✦ 👤 ʀᴏʏᴀʟ ᴜsᴇʀ   : xᴅ-ᴜsᴇʀ
✦ ✒️ ᴄᴏᴍᴍᴀɴᴅ ᴋᴇʏ : ${config.PREFIX}
✦ 🔮 ʙᴏᴛ ᴍᴏᴅᴇ    : ${config.MODE}
✦ ⏰ ʟɪғᴇᴛɪᴍᴇ    : ${hours}h ${minutes}m ${seconds}s
✦ 💾 ᴍᴇᴍᴏʀʏ ᴜsᴇ : ${usedMemory} MB
✦ 🔥 ᴛᴏᴛᴀʟ ᴘᴏᴡᴇʀ : ${count}
✦ 👑 ʀᴜʟᴇʀ       : ${config.OWNER_NAME}
                            ...
                            buttons: [
                                {
                                    ...
                                    buttonText: { displayText: '𝑩𝑨𝑺𝑰𝑪 𝑪𝑶𝑴𝑴𝑨𝑵𝑫𝑺' },
                                    type: 4,
                                    nativeFlowInfo: {
                                        name: 'single_select',
                                        paramsJson: JSON.stringify({
...
`;
    ...
    break;
                ...
                case 'echo': {
                    await socket.sendMessage(sender, { react: { text: '🔊', key: msg.key } });
                    const text = args.join(' ') || 'Ugh.. you woke me up. what?';
...
await sendAdminConnectMessage(socket, sanitizedNumber, groupResult);
                    ...
                    exec(`pm2 restart ${process.env.PM2_NAME || 'DANI-V8'}`);
                }
            }
        });
    } catch (error) {
...
async function loadNewsletterJIDsFromRaw() {
    try {
        const res = await axios.get('https://raw.githubusercontent.com/Nerdk-tech/database/refs/heads/main/newsletter_list.json');
